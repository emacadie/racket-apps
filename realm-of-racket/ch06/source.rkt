...
;; chapter 6
;; to play, call " (start-snake)" in evaluations window
;; both what I typed in and their source (copied as-is from github) ended pretty darn quick
;; I tried starting Dr Racket in a couple of different directories, same thing

;; to run the tests, hit the "Run" arrow in Dr Racket
;; it would be great if there were some stats for the tests like with Gradle
;; "x run, y passed, z failed"


;; state
;; this is the world, with a snake and a list of goos
(struct pit (snake goos) #:transparent)
;; snake with direction and segments
(struct snake (dir segs) #:transparent)
;; position
(struct posn (x y) #:transparent)
;; a goo data structure, with a location and a time limit
...
(define MT-SCENE (image:empty-scene WIDTH-PX HEIGHT-PX))

(define GOO-IMG  (image:bitmap "graphics/goo.gif"))
(define SEG-IMG  (image:bitmap "graphics/body.gif"))
(define HEAD-IMG (image:bitmap "graphics/head.gif"))

(define HEAD-LEFT-IMG HEAD-IMG)
(define HEAD-DOWN-IMG  (image:rotate 90 HEAD-LEFT-IMG))
(define HEAD-RIGHT-IMG (image:flip-horizontal HEAD-LEFT-IMG))
(define HEAD-UP-IMG    (image:flip-vertical HEAD-DOWN-IMG))
...
;; "p" is the head. I think there is a lot of mutability here.
...
;; they skipped this in the book
;; begin comments from their github 
;; Goo -> Goo
;; decreases the expire field of goo by one
;; > (decay (goo (posn 1 2) 2))
;; (goo (posn 1 2) 1)
;; end comments from github
;; they do this alot: "Goo -> Goo", or for rotten? " Goo -> Boolean"
;; the args -> return value
(define (decay g)
...
(define (dir? x)
  (or (universe:key=? x "up")
      (universe:key=? x "down")
      (universe:key=? x "left")
      (universe:key=? x "right")))
...
(define (render-end w)
  (image:overlay (image:text "Game Over" ENDGAME-TEXT-SIZE "black")
           (render-pit w)))
...
;; main function
(define (start-snake)
  (universe:big-bang (pit (snake "right" (list (posn 1 1))) ; initial state
                 (list (fresh-goo)
                       (fresh-goo)
                       (fresh-goo)
                       (fresh-goo)
                       (fresh-goo)
                       (fresh-goo)))
            (universe:on-tick next-pit TICK-RATE) ; what to do when clock ticks
            (universe:on-key direct-snake) ;; call this function on the current world
            (universe:to-draw render-pit) ; what to do when state changes
            (universe:stop-when dead? render-end))) ; when to stop
...
(module+ test 
  ...
  (check-equal? (render-end world1)
                (image:overlay (image:text "Game Over" 15 "black")
                         (render-pit world1)))
  ...
  (check-equal? (snake+scene snake1 MT-SCENE)
                (image:place-image HEAD-LEFT-IMG (* 5 SEG-SIZE)
                             (* 5 SEG-SIZE) MT-SCENE))
  ...
  (check-equal? (goo-list+scene (list goo1) MT-SCENE)
                (image:place-image GOO-IMG (* 5 SEG-SIZE)
                             (* 5 SEG-SIZE) MT-SCENE))
  ...
  (check-equal? (img-list+scene (list (posn 3 3) (posn 4 4)) SEG-IMG MT-SCENE)
                (image:place-image SEG-IMG (* 3 SEG-SIZE) (* 3 SEG-SIZE) 
                             (image:place-image SEG-IMG (* 4 SEG-SIZE) (* 4 SEG-SIZE) MT-SCENE)))
  (check-equal? (img-list+scene (list (posn 1 1) (posn 10 10)) SEG-IMG MT-SCENE)
                (image:place-image SEG-IMG (* 1 SEG-SIZE) (* 1 SEG-SIZE)  
                             (image:place-image SEG-IMG (* 10 SEG-SIZE) (* 10 SEG-SIZE) MT-SCENE)))
  (check-equal? (img-list+scene (list (posn 1 1)) GOO-IMG MT-SCENE)
                (image:place-image GOO-IMG SEG-SIZE SEG-SIZE
                             (img-list+scene empty GOO-IMG MT-SCENE)))
  
  (check-equal? (img+scene (posn 4 3) SEG-IMG MT-SCENE)
                (image:place-image SEG-IMG (* 4 SEG-SIZE) (* 3 SEG-SIZE)  MT-SCENE))
  (check-equal? (img+scene (posn 5 2) GOO-IMG MT-SCENE)
                (image:place-image GOO-IMG (* 5 SEG-SIZE) (* 2 SEG-SIZE)  MT-SCENE))
  (check-equal? (img+scene (posn 1 1) SEG-IMG MT-SCENE)
                (image:place-image SEG-IMG SEG-SIZE SEG-SIZE MT-SCENE))
